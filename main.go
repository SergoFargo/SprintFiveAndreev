...
type InfoMessage struct {
	// добавленны необходимые поля в структуру
	TrainingType string        // тип тренировки
...
func (t Training) TrainingInfo() InfoMessage {
	// ниже мой код
	return InfoMessage{
		TrainingType: t.TrainingType, // получим тип тренировки
		Duration:     t.Duration,     // получим продолжительность тренировки
		Distance:     t.distance(),   // получим дистанцию тренировки
		Speed:        t.meanSpeed(),  // получим среднюю скорость
		Calories:     t.Calories(),   // получим каллории
	}
}
...
type CaloriesCalculator interface {
	// добавлены необходимые методы в интерфейс
	Calories() float64
...
type Walking struct {
	// добавлены необходимые поля в структуру
	Training
...
func (w Walking) Calories() float64 {
	// ниже мой код
	if w.Training.meanSpeed() == 0 || w.Height == 0 {
		return 0
	}
...
func (s Swimming) meanSpeed() float64 {
	...
	if s.LengthPool == 0 || s.CountPool == 0 || s.Training.Duration.Hours() == 0 {
		return 0
	}
...
func (s Swimming) TrainingInfo() InfoMessage {
	// вставьте ваш код ниже
	return InfoMessage{
		TrainingType: s.TrainingType, // получим тип тренировки - плавание
		Duration:     s.Duration,     // получим продолжительность заплыва
		Distance:     s.distance(),   // получим дистанцию заплыва
		Speed:        s.meanSpeed(),  // получим среднюю скорость при плавание
		Calories:     s.Calories(),   // получим потраченые калории при плавании
	}
}
